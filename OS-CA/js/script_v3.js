	...
	$(document).ready(function () {
		...
		function contactSubmit() {
			$('.submit-contact').on('click', function () {
				...
				if (lang === 'tr' || lang === 'en') {
					window.open('mailto:satis@os-ca.com?subject=OSCA Anfrage&body='+encodeURIComponent(content));
				  } else {
					// If it's German, use info
					window.open('mailto:info@os-ca.com?subject=OSCA Anfrage&body='+encodeURIComponent(content));
				  }
				return false;
			});
		}
...
document.querySelectorAll('.lang-option').forEach(function(element) {
    element.addEventListener('click', function(e) {
      ...
      const lang = this.getAttribute('data-lang'); // Get language from data-lang attribute
      let currentUrl = window.location.href; // Get current URL

      // Remove the current language code from the URL if it exists
      if (currentUrl.includes('/tr') || currentUrl.includes('/en') || currentUrl.includes('/de')) {
        currentUrl = currentUrl.replace(/\/(tr|en|de)/, ''); // Remove /tr, /en, or /de part
      }

      // Now append the language code before the file name (if it's Turkish or English)
      if (lang === 'tr' || lang === 'en') {
        window.location.href = currentUrl.replace(window.location.origin, window.location.origin + '/' + lang);
      } else {
        // If it's German, remove the language prefix from the URL
        window.location.href = currentUrl.replace(window.location.origin + '/de', window.location.origin);
      }
    });
  });